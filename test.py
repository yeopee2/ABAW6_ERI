...
# +
def save_result_csv(ckpt_fold, video_names, output):

    head = ['File_ID', 'Adoration', 'Amusement', 'Anxiety', 'Disgust', 'Empathic-Pain', 'Fear', 'Surprise']
    
    save_path = './Test_Results/'

    predict_save_path = save_path + f'/{ckpt_fold}_predicted.csv'
...
#         test_target += y_label.cpu().tolist()
    ...
    save_result_csv(args.ckpt_fold, test_video_name, test_predict)
...
#     ckpt_dir = '/abaw5/MTL_abaw5/tmp_results/'
    ...
    if args.task is not None:
        ...
        folds = sorted(glob.glob(ckpt_dir + f'/*'))
